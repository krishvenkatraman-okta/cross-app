...
export default function JarvisPage() {
  ...
  const performTokenExchange = async () => {
    try {
      console.log("[v0] Starting JAG token exchange for inventory access...")

      const storedTokens = localStorage.getItem("okta_tokens")
      if (!storedTokens) {
        console.error("[v0] No stored tokens found in localStorage")
        throw new Error("No stored tokens found. Please sign in again.")
      }

      const tokens = JSON.parse(storedTokens)
      const idToken = tokens.id_token
      if (!idToken) {
        console.error("[v0] No ID token found in stored tokens")
        throw new Error("No ID token found. Please sign in again.")
      }

      console.log("[v0] Found ID token, length:", idToken.length)
      ...
      const tokenEndpoint = "https://fcxdemo.okta.com/oauth2/v1/token"

      console.log("[v0] Using token endpoint:", tokenEndpoint)
      console.log("[v0] Making JAG token exchange request...")
      ...
      console.log("[v0] JAG request parameters:", {
        grant_type: "urn:ietf:params:oauth:grant-type:token-exchange",
        requested_token_type: "urn:ietf:params:oauth:token-type:id-jag",
        subject_token_type: "urn:ietf:params:oauth:token-type:id_token",
        audience: "http://localhost:5001",
        client_id: process.env.NEXT_PUBLIC_OKTA_JARVIS_CLIENT_ID,
        has_client_secret: !!process.env.OKTA_JARVIS_CLIENT_SECRET,
        subject_token_length: idToken.length,
      })

      const jagResponse = await fetch(tokenEndpoint, {
        method: "POST",
        credentials: "include",
        headers: {
      ...
      console.log("[v0] JAG response status:", jagResponse.status, jagResponse.statusText)

      if (!jagResponse.ok) {
        const errorText = await jagResponse.text()
        console.error("[v0] JAG token exchange failed:", {
          status: jagResponse.status,
          statusText: jagResponse.statusText,
          body: errorText,
        })
        throw new Error(`JAG token exchange failed: ${jagResponse.status} ${errorText}`)
      }

      const jagResult = await jagResponse.json()
      console.log("[v0] JAG token exchange successful:", {
        issued_token_type: jagResult.issued_token_type,
        token_type: jagResult.token_type,
        expires_in: jagResult.expires_in,
        access_token_length: jagResult.access_token?.length,
      })

      console.log("[v0] Making inventory access token exchange...")
      ...
      console.log("[v0] Inventory response status:", inventoryResponse.status, inventoryResponse.statusText)

      if (!inventoryResponse.ok) {
        const errorText = await inventoryResponse.text()
        console.error("[v0] Inventory token exchange failed:", {
          status: inventoryResponse.status,
          statusText: inventoryResponse.statusText,
          body: errorText,
        })
        throw new Error(`Inventory token exchange failed: ${inventoryResponse.status} ${errorText}`)
      }

      const inventoryResult = await inventoryResponse.json()
      console.log("[v0] Inventory token exchange successful:", {
        token_type: inventoryResult.token_type,
        expires_in: inventoryResult.expires_in,
        scope: inventoryResult.scope,
      })
